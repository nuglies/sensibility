...
router.post('/sensors/:sensor/sensorsettings', function(req, res, next) {
  ...
  SensorSettings.sensor = req.sensor;
	...
	console.log(sensorsettings._id);
  
  	
  	results = SensorSettings.find({'sensorsettings.$id': sensorsettings._id, 'growState': 'grow'}).count;
  	...
  	var query = {'sensorsettings.$id': sensorsettings._id, 'growState': 'grow'};
	req.newData = sensorsettings;
	SensorSettings.findOneAndUpdate(query, req.newData, {upsert:true, 'new': true}, function(err, doc){
